...
public class Student {
    ...
    public static Student getRandomStudent(Course... allCourses) {
        Random r = new Random();

        int id = r.nextInt(10000, 99999);
        String[] countries = {"US", "UK", "DE", "IN", "PL", "CA", "FR"};
        String country = countries[r.nextInt(countries.length)];
        int yearEnr = r.nextInt(Year.now().getValue() - 3, Year.now().getValue() + 1);
        int age = r.nextInt(18, 65);
        String gender = r.nextBoolean() ? "M" : "F";
        boolean exp = r.nextBoolean();

        int count = r.nextInt(1, Math.min(4, allCourses.length + 1));
        Collections.shuffle(Arrays.asList(allCourses));
        Course[] selected = Arrays.copyOfRange(allCourses, 0, count);

        Student s = new Student(id, country, yearEnr, age, gender, exp, selected);

        for (Course c : selected) {
            int lecture = 1 + r.nextInt(Math.max(1, c.getLectureCount()));
            int year = r.nextInt(yearEnr, Year.now().getValue() + 1);
